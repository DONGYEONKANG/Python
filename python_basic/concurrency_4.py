...
# GIL : 두 개 이상의 스레드가 동시에 실행 될 때 하나의 자원을 엑세스 하는 경우 -> 문제점을 방지하기 위해 GIL 실행, 리소스 전체에 락이 걸린다.
# -> Context Switch(문맥교환)
...
WORK_LIST  = [100000, 100000, 1000000, 10000000]
...
def main():
    ...
    with futures.ThreadPoolExecutor() as excutor:
        # map -> 작업 순서가 유지, 즉시 실행
        result = excutor.map(sum_generator, WORK_LIST) # 각각 숫자가 들어간다.
